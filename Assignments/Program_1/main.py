...
class DrawGeoJson(object):
    ...
    def __init__(self, screen, width, height):
        ...
        self.mapLatBottomDegree = self.mapLatBottom * \
            math.pi / 180.0  # bottom in degrees

        self.colors = Colors(
            'C:/Users/Aalat/Documents/4553-Spatial-DS/Resources/Json_Files/colors.json')
    ...
    def convertGeoToPixel(self, lon, lat):
        ...
        self.worldMapWidth = (
            (self.mapWidth / self.mapLonDelta) * 360) / (2 * math.pi)
        self.mapOffsetY = (self.worldMapWidth / 2 * math.log((1 + math.sin(
            self.mapLatBottomDegree)) / (1 - math.sin(self.mapLatBottomDegree))))
        y = self.mapHeight - ((self.worldMapWidth / 2 * math.log(
            (1 + math.sin(lat)) / (1 - math.sin(lat)))) - self.mapOffsetY)
