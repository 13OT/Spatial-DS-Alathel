...
class MongoHelper(object):
    ...
    def get_features_near(self,collection,point,field,value,min_max,radius,earth_radius=3963.2): #km = 6371
        ...
        if min_max.lower()=='min':
            ...
            res = self.client['world_data'][collection].find( { 'geometry': { '$geoWithin': { '$centerSphere': [ [x, y ] , radius/earth_radius ] } },prop:{'$gte':value}})

        else:
            res = self.client['world_data'][collection].find( { 'geometry': { '$geoWithin': { '$centerSphere': [ [x, y ] , radius/earth_radius ] } },prop:{'$lte':value}})
    ...
    def get_feature_in_poly(self,collection,poly):
        ...
        return self._make_result_list(res)
    def get_airport_coord(self,iata):
        res = self.client['world_data']['airports'].find_one({'properties.ap_iata' : iata},{'_id':0})
        return res['geometry']['coordinates']
    
    def get_airport_near_me(self,point,radius):
        x,y = point

        res = self.client['world_data']['airports'].find( { 'geometry': { '$geoWithin': { '$centerSphere': [ [x, y ] , radius/3963.2 ] } }} )
